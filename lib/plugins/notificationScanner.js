...
const scanNotifications = server => {
  ...
  const debugLog = message => {
    if (process.env.DEBUG_SCANNER.toLowerCase() === 'true') {
      server.log(['debug', 'notification-scanner'], message)
    }
  }
  ...
  const createNotificationsForSubscriptions = async ({
    ...
    // We filter events without a subscription first to avoid making unnecessary getBlock calls
    ...
    debugLog(`events with matching subscriptions: ${eventsWithSubs.length}`)
    ...
    return Promise.all(
      decoratedEventsWithSubs.flatMap(([event, matchedSubscriptions]) => {
        debugLog(
          `creating ${matchedSubscriptions.length} notifications for event ${event.event}`
        )
  ...
  return async () => {
    ...
    const latestBlock = {}
    try {
      latestBlock[NETWORK_MAINNET] = await server.app.web3.getLatestBlock(
        NETWORK_MAINNET
      )
      latestBlock[NETWORK_RINKEBY] = await server.app.web3.getLatestBlock(
        NETWORK_RINKEBY
      )
    } catch (error) {
    ...
    for (const contract of contractsToScan) {
      ...
      const web3Contract = getContract({ abi, contractAddress, network })
      ...
      try {
        ...
        debugLog(
          `Scanned contract ${contractAddress} fromBlock: ${fromBlock}| Event count: ${events.length}`
        )
      } catch (error) {
        web3ErrorHandler(error, network)
        continue
      ...
      if (events.length > 0) {
        ...
        try {
          ...
          debugLog(`creating ${notifications.length} notifications`)
          notificationsCounter.inc(notifications.length)
