...
const scanNotifications = server => {
   ...
   */
  const createNotifications = ({ db, events, subscriptions }) => {
   ...
   * @param {string} options.contractAddress  a knex db or transaction instance
   */
  ...
  return async () => {
    ...
    for (const contract of contractsToScan) {
      ...
      if (events.length > 0) {
        ...
        trx.executionPromise.catch(error => {
          // you get here only if a commit or a rollback fails
          console.error('shiiiit', error)
        })

        try {
          await createNotifications({ db: trx, events, subscriptions })
          await updateEventsources({ db: trx, contractAddress, latestBlock })
          await trx.rollback()
        } catch (error) {
          // await trx.rollback()
