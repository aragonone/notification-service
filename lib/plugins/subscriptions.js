...
const debugLog = (request, message) => {
  if (process.env.DEBUG_SUBSCRIPTIONS.toLowerCase() === 'true') {
    request.log(['debug', 'subscriptions'], message)
  }
}
  ...
  try {
    ...
    if (memoizedNamehash(appName) !== appId) {
      return Boom.badRequest(
        "Namehash of appName does not match the contractAddress' appId"
      )
    }
    ...
    if (ensName && resolvedEnsAddress !== kernelAddress) {
      return Boom.badRequest(
    ...
    debugLog(
      request,
      `Created subscription ${subscriptionId} | contract: ${contractAddress} | event name: ${eventName} | current block: ${latestBlock}`
    )
...
async function deleteSingleSubscription(request, h) {
  ...
  try {
    ...
    debugLog(request, `Deleted single subscription ${subscriptionId}`)
...
async function deleteSubscriptions(request, h) {
  ...
  try {
    ...
    debugLog(request, `Deleted all subscriptions for user ${userId}`)
    return h.response(deleteCount).code(200)
  } catch (error) {
