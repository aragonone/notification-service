...
async function getSubscriptions(request, h) {
  ...
  const { network, contractAddress, kernelAddress, appName } = request.query
  try {
...
async function createSubscription(request, h) {
  const {
    db,
    web3: { getKernelForApp, resolveEnsDomain },
  } = request.server.app
  const { userId } = request.auth.credentials
  ...
  try {
    ...
    let kernelAddress, resolvedEnsAddress
    try {
      // get kernel for the app contract and verify it matches the resolved ENS address
      ;[kernelAddress, resolvedEnsAddress] = await Promise.all([
        getKernelForApp({ contractAddress, network }),
        resolveEnsDomain({ ensName, network }),
      ])
    } catch (error) {
    ...
    if (resolvedEnsAddress !== kernelAddress)
      return Boom.badRequest(
        `ENS name does not reolve to a valid kernel. Resolved address: ${resolvedEnsAddress} Kernel: ${kernelAddress}`
      )
