...
import { NOTIFICATION_MAILER_INTERVAL } from '../constants'

const notificationMailer = {
  name: 'ns/notification-mailer',
  version: '1.0.0',
  register: async function(server, options) {
    server.dependency(['ns/metrics', 'ns/database', 'ns/mail'])
    const notificationMailerFn = sendNotifications(server)

    // eslint-disable-next-line no-unused-vars
    const notifications = new Scheduler(
      notificationMailerFn,
      NOTIFICATION_MAILER_INTERVAL
    )
  ...
  const getNotificationToSend = () =>
    db('notifications')
      .select(
        ...
        'notifications.block',
        'notifications.block_time as blockTime',
      ...
      .orderBy('notifications.block', 'asc') // process older notifications first
