...
function getClientUrl(network) {
  return `https://${network === 'mainnet' ? '' : 'rinkeby.'}client.aragon.org`
}
  ...
  return async ({ refererHost, email, dao, network, token }) => {
    ...
    if (refererHost && ALLOWED_HOSTNAMES.includes(refererHost)) {
      ...
      actionUrl = new URL(
        `${getClientUrl(
          network
        )}/#/${dao}/?preferences=/notifications/verify/${token}`
      ).toString()
    }
...
function sendNotificationEmail(server) {
  return async ({
    ...
    const orgAddress = ensName || kernelAddress
    const orgUrl = `${getClientUrl(network)}/#/${orgAddress}`

    const managementUrl = `${orgUrl}/?preferences=/notifications`
    ...
    const truncateAddress = addr => `${addr.substr(0, 6)}...${addr.substr(-4)}`
    ...
    const processAppTitle = (appName, address) =>
      `${capitalize(appName.replace('.aragonpm.eth', ''))} (${truncateAddress(
        address
      )})`
