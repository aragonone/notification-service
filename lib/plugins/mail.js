...
function sendMagicLink(server) {
  return async ({ refererHost, email, dao, network, token }) => {
    ...
    if (ALLOWED_HOSTNAMES.includes(refererHost)) {
...
function sendNotificationEmail(server) {
  return async ({
    ...
    You are receiving this email because you are subscribed to Aragon Email Notifications.
    <a href="${managementUrl}">Change your email notifications settings</a> if you no longer wish to receive these.
