...
export async function getAbi(contractAddress) {
  const appProxyBaseABI = require('@aragon/os/abi/AppProxyBase.json').abi
  const appContract = new web3.eth.Contract(appProxyBaseABI, contractAddress)
...
export async function getAbiFromApmServe(appName) {
  const apmUrl = getApmUrl({ app: appName })
  try {
